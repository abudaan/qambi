...
var defaultMetronomeInstrument = null;
function getDefaultInstrument() {
  if (defaultMetronomeInstrument === null) {
    var data = (0, _init_audio.getInitData)();
    defaultMetronomeInstrument = new _instrument.Instrument('metronome');
      ...
      if (this.song._loop === true) {
        ...
        if (this.maxtime >= this.song._rightLocator.millis && this.beyondLoop === false) {
          ...
          if (this.looped === false) {
            (function () {
              _this.looped = true;
              for (var i = _this.index; i < _this.numEvents; i++) {
                var event = _this.events[i];
                if (event.millis < _this.song._rightLocator.millis) {
                  event.time = _this.timeStamp + event.millis - _this.songStartMillis;
                  events.push(event);
                  _this.index++;
                } else {
                  break;
              ...
              var endTicks = _this.song._rightLocator.ticks - 1;
              var endMillis = _this.song.calculatePosition({ type: 'ticks', target: endTicks, result: 'millis' }).millis;

              var updated = [];
              var leftMillis = _this.song._leftLocator.millis;
              var rightMillis = _this.song._rightLocator.millis;

              events.forEach(function (event) {
                if (event.type === 128 && event.millis > rightMillis) {
                  var clone = new _midi_event.MIDIEvent(endTicks, 128, event.data1, 0);
                  clone.millis = endMillis;
                  clone.midiNote = event.midiNote;
                  clone._track = event._track;
                  clone.time = _this.timeStamp + clone.millis - _this.songStartMillis;
                  updated.push(clone);
                } else {
                  updated.push(event);
                }
              ...
              */
              _this.setIndex(leftMillis);
              _this.timeStamp += _this.song._loopDuration;
